...
class CommitteeCrudModalNotExtended extends React.Component {
    ...
    onSubmit = () => {
        ...
        if ( isFormValid ) {
            ...
            if ( undefined === id ) {
                new ApiCommittee().addNewCommitteeItem( dataToDb );
            } else {
                new ApiCommittee().updateCommitteeItem( dataToDb );
            }
    ...
    render() {
        ...
        const modalTitle = `${undefined !== id ? t( 'Edit' ) : t( 'New' )}` + ' ' + `${t( 'Committee' )}`;
...
CommitteeCrudModalNotExtended.propTypes = {
    id: PropTypes.string,
    show: PropTypes.bool,
...
const CommitteeCrudModal = hoistStatics( withTranslation()( CommitteeCrudModalNotExtended ), CommitteeCrudModalNotExtended );

export default connect( mapStateToProps, mapDispatchToProps )( CommitteeCrudModal );
